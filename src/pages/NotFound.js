  ...
  return (
    <div className="min-h-screen bg-[#050508] text-white flex flex-col items-center justify-center p-6 text-center font-sans overflow-hidden relative">
      ...
      <div className="relative z-10 max-w-lg px-4">
        ...
        <h2 className="text-2xl md:text-4xl font-black mb-4 md:mb-6 tracking-tight uppercase leading-tight">{interpolate(content.ui?.routingError, content) || 'Сбой маршрутизации'}</h2>
        <p className="text-slate-400 text-xs md:text-sm leading-relaxed mb-10 md:mb-12 font-medium">
          {interpolate(content.ui?.pageNotFound, content) || 'Запрошенная страница не существует или была перемещена в архив. Пожалуйста, вернитесь на главную панель управления.'}
        </p>
        ...
        >
          <Home size={20} className="group-hover:-translate-y-0.5 transition-transform" /> {interpolate(content.ui?.backToStart, content) || 'Вернуться в начало'}
        </Link>
      </div>
      ...
      <footer className="absolute bottom-12 left-0 right-0 text-[9px] font-bold uppercase tracking-[0.3em] text-slate-700 select-none">
        System Status: Error 0x404 • URL_NOT_FOUND • v{interpolate(version, content)}
      </footer>
    </div>
  );
}
