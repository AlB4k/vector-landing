      ...
      <div className="max-w-4xl mx-auto">
        ...
        <section className={`${isLight ? 'bg-slate-50 border-slate-200 shadow-lg' : 'bg-slate-900/30 border-blue-500/20 shadow-2xl'} p-6 md:p-10 rounded-3xl md:rounded-[2.5rem] border mb-8 md:mb-12`}>
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 sm:gap-6 mb-8 text-center sm:text-left">
            ...
            <div>
              <h2 className={`text-lg md:text-xl font-black ${isLight ? 'text-slate-900' : 'text-white'} mb-3 md:mb-4 tracking-tight uppercase`}>Уведомление об отказе от публичной оферты</h2>
              <p className={`text-xs md:text-sm leading-relaxed ${isLight ? 'text-slate-500' : 'text-slate-400'} italic opacity-70`}>В соответствии со ст. 437 Гражданского кодекса Российской Федерации</p>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className={`p-8 ${isLight ? 'bg-slate-50 border-slate-200' : 'bg-slate-900/20 border-slate-800/50'} rounded-3xl border flex flex-col justify-center text-center`}>
            <p className={`text-[10px] font-black uppercase ${isLight ? 'text-slate-400' : 'text-slate-500'} tracking-widest mb-6`}>Нужен индивидуальный расчет?</p>
